...
```
Объявляется, но не определяется, шаблонная функция, принимающая ссылку на массив от числа N и типа T элементов:
...
Показывается, что это используется в compile-time С++98. Мы же этого показывать не будем, а продемонстрируем, что это решение работает и по сей
день:
...
Для удобства пользования (и ни для чего большего) определяется макрос для использования функции и структуры. И результат по мановению рук
превращается в вариант Мейерса:
