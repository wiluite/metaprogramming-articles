...
создать объект-кортеж, заполнив его данными из объекта-структуры (и впоследствии работая и с ними и с объектом-структорой через интерфейс
кортежей), не обращаясь к членам по имени. Например, кортеж и структура таковы:
...
как там размещение внутри std::tuple может быть совсем иным, а то и размер кортежа не соответствовать сумме размеров его членов).*
...
первого члена работает, потому что саму структуру компилятор выравнивает по (правильному) умолчанию.
...
#### 3.3. Создание кортежей со списком типов POD-структуры.
...
#### 4. О использовании constexpr в constexpr-функциях.
...
initializer_list от int это constant expression? Нет, по синтаксису это не core constant expression. Всё, оно не может быть использовано, чтобы
инициализировать core constant expression. Синтаксическая проверка не проходит, и это очень важно. Констэкспры почти всегда хотят
синтаксической проверки. Это немножко облегчает реализацию компилятора.".*
...
#### 5. Compile-time определитель размера массива.
...
#### 6. Нерекурсивный разворот списка. История reverse и 2-3 решения вопроса.
...
В этом разделе статьи надо решить 4 проблемы: 

* улучшение асимптотического времени алгоритма
...
то всё вместе, это уже квадратичный алгоритм. Однако, многие compile-time алгоритмы могут получать пользу от того, что некоторые их части 
...
гарантированно линейную реализацию, опирающуюся не на классический алгоритм с вставкой в конец, а на некую ремесленную альтернативу с
привлечением кортежей, и приводить ее я здесь не буду, хотя и добавлю в репозитарий файл example_6_O(n).cpp.

Итак, мы резюмируем, что алгоритм разворота списка - всегда линейный, его реализация такова во всех современных реализациях, и тем самым мы
закрываем первую проблему.

Далее. Если взглянуть на все ту же реализацию из Boost.mp11, то автор библиотеки так же позаботился об уменьшении количества инстанцирований
промежуточных типов при вычислениях, делая разворот списков "с шагом 10". То есть, в рекурсивном случае осуществляется работа не с одним
...
возможность работы с огромными количествами типов в списках типов, не применяя рекурсию. Отсутствие же рекурсии само по себе гарантирует
увеличение скорости компиляции, поскольку новые типы создаются лишь по минимуму и не являются необъемлемыми элементами каждого шага алгоритма,
как ранее.
...
однозначных идентификаторах, и лишь в конце вытаскивались сами типы. Сейчас нам нужно написать императивную итеративную версию разворота списка,
но сделать это так, чтобы она работала внутри constexpr-функций. И вот как выглядит первая попытка:
...
```
В верхней функции массив types сначала заполняется идентификаторами всех типов из списка типов, затем происходит разворот порядка этого набора
на противоположный и, наконец, построение кортежа этих, уже развернутых, идентификаторов с целью возврата клиенту. Вторая сверху функция
...
рекурсивности. Мы берем класс sequence_tuple::tuple из проекта Boost.pfr (magic_get), пишем для него функцию-фабрику и заменяем нашу реализацию:
...
конечно же, некоторого минимального значения, необходимого для работы всего остального).
...
скорости компиляции нескольких альтернатив. И, третье - мы можем работать только с теми типами, которые зарегистрированы в нашей системе
